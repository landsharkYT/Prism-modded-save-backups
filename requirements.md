# Requirements for Prism modded save backups
...
- Reusable entry: `git clone` this repo (or extract its zip) into any Prism instance dir, run `./install.sh`. Clone and zip behave the same; both gated by root resolution (FR-10). A stray subfolder placement is recovered, not punished.
...
- **FR-18 Instance root resolution + layout gate (both scripts, shared helper logic).** The `Instance Root` is the nearest ancestor-or-self of the script's own dir (search up no more than 3 levels) containing `minecraft/saves/`. Found above the script dir, the script adopts it with a loud NOTICE naming the adopted root and continues. None found, the script aborts with exit 2, listing every directory checked plus the fix ("clone/extract into your instance folder"). Never search downward. The working directory is irrelevant. Everything derives from the script's location, never `$PWD`.

## 4. Storage / bloat constraints

- Live world 17M becomes a ~6.7M zip. `KEEP=5` is about 35M in Drive. Drive has 5 TiB total / 2.68 GiB used. Cloud bloat is negligible here; the cap is hygiene, not capacity.
- Local disk `/` is 84% full: script must add 0 bytes net on success (temp in `/tmp`, deleted).
- `minecraft/backups/` (34M, 5x FTB zips) and `minecraft/` (640M total) are explicitly out of git (see `.gitignore`) and out of Drive upload.
...
- [x] Second run overwrites `-latest.zip` (new timestamp/size) and Drive holds no more than 5 dated zips. (2 dated + latest after 2nd run)
...
- [x] Scripts run from a repo subfolder auto-adopt the instance root no more than 3 levels up with a NOTICE line (bats 9 + 35, both scripts).
...
- **FR-11 Dependency check, no auto-install.** Require `bash`/`zip`; resolve `rclone` per FR-12. `unzip`/`python3`/`bats` optional (`bats` absent = warn-only skip). Anything required and missing means printing the exact per-distro install command (`pacman`/`apt`/`dnf`/`brew`, plus rclone's official install script) and exit 2. Never `sudo`, never install.
- **FR-12 rclone discovery.** Candidates in order: `PATH`, then `$HOME/.local/bin/rclone`. Echo resolved path + `rclone version`. Ask y/n to use it (`--yes` implies yes). None found, print the FR-11 hints + exit 2.
- **FR-13 Remote triage.** `rclone listremotes`, Drive-type only: zero goes to the auth path (FR-14); exactly one gets y/n "use `<name>:`?" (no goes to the auth path); several get a numbered pick-list + a "set up new" option. Non-Drive remotes are never offered.
- **FR-14 Auth loop.** Launch interactive `rclone config`, then re-detect and prove with `rclone about <remote>`. Failure means abort, no partial state. No TTY and no `--yes` means printing manual steps instead, exit 2. With `--yes`, attempt the config once. A stubbed or piped config can still succeed. A real closed-stdin run prints the manual steps instead.
...
- **FR-16 Verification + installer log.** `chmod +x` both scripts, `bash -n` both (abort on failure), `bats tests/` if bats exists else warn. Append the run transcript to `install.log` next to the script (same tee shape as FR-9; gitignored). Offer the first real backup, y/n, default No. `--yes` never implies it.
